...
namespace fluid {
	...
	private:
		...
		inline static T *_alloc(size_type count) {
			...
			// POSIX 平台使用 posix_memalign
			void* ptr = nullptr;
			if (posix_memalign(&ptr, alignof(T), sizeof(T) * count) != 0) {
				return nullptr; // 分配失败返回 nullptr
			}
			return static_cast<T*>(ptr);
#else
			// 其他平台，使用 std::malloc 实现对齐
			...
			void* aligned = reinterpret_cast<void*>(
				(reinterpret_cast<std::size_t>(original) + alignment - 1 + sizeof(void*)) & ~(alignment - 1)
				);
		...
		inline static void _free(T *ptr) {
			...
			::_aligned_free(ptr);
#elif defined(__unix__) || defined(__APPLE__)
			free(ptr); // posix_memalign 使用 free
#else
			if (ptr) {
