...
namespace fluid {
	...
	public:
						...
						for (std::size_t sx = 0; sx < density; ++sx) {
							for (std::size_t sy = 0; sy < density; ++sy) {
								for (std::size_t sz = 0; sz < density; ++sz) {
									...
									//std::cout << position.x << ' ' << position.y << ' ' << position.z << " £º" << pred(position) << std::endl;
									if (pred(position)) {
										particle p;
		...
		/// Seeds the simulation with water particles in the given specific area.
		template <typename Func>
		void seed_area(
			vec3d start, vec3d size, const Func& is_seed, vec3d velocity = vec3d(), std::size_t density = default_seeding_density
		) {
