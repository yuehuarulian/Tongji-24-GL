...
/*
    文件解析：Config 类通过正则表达式解析配置文件中的不同类型的数据（如字符串、整数、浮动数值、十六进制、二进制等）。
    数据存储：解析后的数据被存储在不同的 LStorage 容器中，按照类型进行分类。
    数据访问：提供了多种方法来访问这些存储的数据，支持通过 id 获取不同类型的值。
    配置有效性：通过 valid 标志位来检查配置是否成功加载。
*/
